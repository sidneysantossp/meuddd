...
function cleanArtifacts(text: string): string {
  return text
    .replace(
      /\w{10,120}\??\s*;\s*na pr[áa]tica[^.]{0,200}\./gi,
      "(ver dados oficiais)."
    )
    .replace(
      /\w{8,80}\??\s*; na verdade[^.]{0,200}\./gi,
      "(ver dados oficiais)."
    )
    ...
    tourism: {
      ...
      properties: {
        intro: {
          type: "string",
          description:
            "Introdução de 140 a 220 palavras sobre o turismo da cidade e seus arredores, com contexto geográfico e histórico breve.",
        },
        items: {
          type: "array",
          items: {
            ...
            properties: {
              name: {
                type: "string",
                description:
                  "Nome do ponto turístico ou atração (público, verificável ou geográfico).",
              },
              description: {
                type: "string",
                description:
                  "Descrição factual de 2 a 4 frases, com contexto prático (visita, época, significado local).",
              },
            },
        ...
        closing: {
          type: "string",
          description:
            "Fecho de 1 a 2 frases com dica prática para o visitante.",
        },
      },
    ...
    dining: {
      ...
      properties: {
        intro: {
          type: "string",
          description:
            "Introdução de 90 a 140 palavras sobre a gastronomia típica da região e seus hábitos alimentares locais.",
        },
        items: {
          type: "array",
          items: {
            ...
            properties: {
              name: {
                type: "string",
                description:
                  "Nome do tipo de prato, tradição ou local gastronômico.",
              },
              kind: {
                type: "string",
                description:
                  "Categoria curta, ex.: 'Prato típico', 'Mercado', 'Feira regional'.",
              },
              description: {
                type: "string",
                description: "Descrição factual de 1 a 3 frases.",
              },
            },
        ...
        closing: {
          type: "string",
          description: "Fecho curto com recomendação prática.",
        },
      },
      required: ["intro", "items", "closing"],
      additionalProperties: false,
    ...
    transport: {
      ...
      properties: {
        intro: {
          type: "string",
          description:
            "Introdução de 80 a 120 palavras sobre como chegar à cidade e como se deslocar dentro dela (ônibus municipal, rodoviária, aeroporto, ferrovias quando existentes).",
        },
        items: {
          type: "array",
          items: {
            ...
            properties: {
              name: {
                type: "string",
                description:
                  "Nome do serviço ou infraestrutura de transporte (rodoviária, aeroporto, terminal, serviço municipal).",
              },
              kind: {
                type: "string",
                description:
                  "Categoria, ex.: 'Rodoviária', 'Aeroporto', 'Ônibus municipal'.",
              },
              description: {
                type: "string",
                description: "Descrição factual de 1 a 3 frases.",
        ...
        closing: {
          type: "string",
          description: "Fecho curto com dica de mobilidade urbana.",
        },
      },
      required: ["intro", "items", "closing"],
      additionalProperties: false,
    ...
    climate: {
      ...
      properties: {
        intro: {
          type: "string",
          description:
            "Introdução de 60 a 100 palavras sobre o clima da região.",
        },
        details: {
          type: "array",
          items: {
            ...
            properties: {
              label: {
                type: "string",
                description:
                  "Rótulo curto, ex.: 'Classificação Köppen', 'Temperatura média anual'.",
              },
        ...
        body: {
          type: "string",
          description:
            "Texto de 100 a 160 palavras sobre estação chuvosa, melhor época para visitar e o que esperar do clima ao longo do ano.",
        },
        source: {
          type: "object",
          description:
            'Fonte de autoridade climática: {label: "Climate-Data.org", href: "https://pt.climate-data.org/america-do-sul/brasil/{uf}/<cidade-estilo-slug>/"}.',
        },
      },
...
const PROMPT = (
  m: any
) => `Você é um redator editorial especializado em cidades brasileiras. Escreva o guia local do município de ${m.name}, no estado de ${m.stateName} (${m.stateUf}), região ${m.region} do Brasil, com DDD ${m.ddd} e população estimada de ${m.populationEstimated !== null ? new Intl.NumberFormat("pt-BR").format(m.populationEstimated as number) : "dados não divulgados"} habitantes${m.capital ? " — capital do estado" : ""}.
...
async function main() {
  ...
  console.log(
    `-- onlyUf=${onlyUf ?? "(todas)"} onlyEmpty=${onlyEmpty} done.size=${done.size}`
  );
  const sample = rows
    .slice(0, 3)
    .map((r: any) => ({ uf: r.stateUf, name: r.name }));
  ...
  console.log(
    `Total na base: ${rows.length}. Na fila: ${queue.length}. A processar agora: ${work.length}.`
  );
      ...
      batch.map(async (r: any) => {
        ...
        try {
          ...
          for (const item of catalog[key].tourism.items)
            item.mapHref = MAP_SEARCH(`${item.name}, ${r.name}, ${r.stateUf}`);
