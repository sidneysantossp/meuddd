...
const pages = [
  ...
  {
    path: "/cidade/sp/aracariguama",
    name: "Município Araçariguama (sem tabs)",
  },
  {
    path: "/cidade/ac/cruzeiro-do-sul",
    name: "Município Cruzeiro do Sul (com tabs)",
  },
...
function fetchHtml(path: string): string {
  try {
    return execSync(`curl -s -m 30 "${BASE}${path}"`, {
...
function extractMeta(
  ...
  const re = new RegExp(
    `<${tagAttr}[^>]*${attr}=["']${value}["'][^>]*content=["']([^"']*)["'][^>]*>`,
    "i"
  );
  const m = html.match(re);
  if (m) return m[1];
  const re2 = new RegExp(
    `<${tagAttr}[^>]*content=["']([^"']*)["'][^>]*${attr}=["']${value}["'][^>]*>`,
    "i"
  );
...
for (const page of pages) {
  ...
  const description =
    extractMeta(html, "name", "description", "meta") ||
    extractMeta(html, "property", "og:description", "meta");
  ...
  results.push({
    ...
    headingsFirst5: headings
      .slice(0, 5)
      .map(h => `${h.level}: ${h.text.slice(0, 70)}`),
...
writeFileSync(
  ".manus-logs/seo-audit-results.json",
...
for (const r of results) {
  console.log(`\n=== ${r.name} (${r.path}) ===`);
  console.log(
    `HTML: ${r.htmlSizeKB}KB | status: ${r.status} | title[${r.titleLen}]: ${r.title.slice(0, 90)}`
  );
  console.log(
    `desc[${r.descLen}]: ${r.description.slice(0, 110)} | canonical: ${r.canonical || "(none)"} | ogImage: ${r.ogImage} | jsonLd: ${r.jsonLdCount} (${r.jsonLdTypes.join(",")})`
  );
  for (const h of r.headingsFirst5) console.log(`   ${h}`);
  console.log(
    `h1: ${r.h1Count} | h2: ${r.h2Count} | internal: ${r.internalLinks} | external: ${r.externalLinks} | nofollow: ${r.nofollowLinks} | textRatio: ${r.textRatio}% | textChars: ${r.textChars}`
  );
}
