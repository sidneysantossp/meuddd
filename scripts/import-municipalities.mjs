...
const sourcePath =
  process.argv[2] ?? "/home/ubuntu/Downloads/ddd-brasil-data/municipios.csv";
...
if (!fs.existsSync(sourcePath))
  throw new Error(`Ficheiro de origem não encontrado: ${sourcePath}`);
...
const rows = fs
  ...
  .map(line => {
    ...
    return [
      ...
      name,
      slugify(name),
      Number(stateIbgeCode),
      String(ddd).padStart(2, "0"),
      latitude,
      longitude,
      timezone,
      capital === "1",
...
const duplicateMunicipalityNames = duplicatesBy(
  ([, name, , stateIbgeCode]) =>
    `${stateIbgeCode}:${name
      .normalize("NFD")
      .replace(/\p{Diacritic}/gu, "")
...
if (duplicateIbgeCodes.length || duplicateMunicipalityNames.length) {
  throw new Error(
    `Fonte contém duplicidades: ${duplicateIbgeCodes.length} código(s) IBGE e ${duplicateMunicipalityNames.length} nome(s) de município por UF.`
  );
}

const ddds = [...new Set(rows.map(([, , , , ddd]) => ddd))].sort(
...
try {
  ...
  await connection.query(
    "INSERT INTO ddd_codes (code) VALUES ? ON DUPLICATE KEY UPDATE code = VALUES(code)",
    [ddds.map(code => [code])]
  );
  ...
  const [[coverage]] = await connection.query(
    "SELECT COUNT(*) AS municipalities, COUNT(DISTINCT stateIbgeCode) AS states, COUNT(DISTINCT ddd) AS ddds FROM municipalities"
  );
    ...
    JSON.stringify(
      {
        sourcePath,
        imported: coverage,
        duplicates: { ibgeCode: 0, municipalityNameWithinState: 0 },
      },
      null,
      2
    )
