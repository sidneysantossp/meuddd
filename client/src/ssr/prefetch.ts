...
import { buildDddFaq, buildMunicipalityFaq, buildStateFaq, faqPageJsonLd } from "@shared/territorialFaq";
...
export async function prefetchForPath(url: string, queryClient: QueryClient, prefetch: SsrPrefetch): Promise<HeadMeta> {
  ...
  if (path === "/gerador") {
    ...
    return { title, description: generatorDescription, canonicalPath: path, ogType: "website", jsonLd: [breadcrumbs([{ name: site, item: "/" }, { name: "Gerador de número de celular", item: path }]), { "@context": "https://schema.org", "@type": "WebPage", name: "Gerador de número de celular por DDD", url: path, description: generatorDescription, about: { "@type": "Thing", name: "Simulação de número de celular brasileiro por DDD" }, isPartOf: { "@type": "WebSite", name: site, url: "/" } }, { "@context": "https://schema.org", "@type": "SoftwareApplication", name: "Gerador de número de celular por DDD", applicationCategory: "UtilitiesApplication", operatingSystem: "Web", url: path, description: generatorDescription, isAccessibleForFree: true, provider: { "@type": "Organization", name: site, url: "/" }, offers: { "@type": "Offer", price: "0", priceCurrency: "BRL" }, featureList: ["Seleção de estado e DDD", "Formatação móvel brasileira com nove dígitos", "Geração exclusivamente simulada para testes"], disclaimer: "A ferramenta não verifica disponibilidade, titularidade ou existência de linhas de telefone." }, { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: [{ "@type": "Question", name: "O número gerado é real ou está disponível?", acceptedAnswer: { "@type": "Answer", text: "Não. A ferramenta constrói apenas um exemplo de formato e não verifica se existe uma linha, se está ativa ou se pertence a alguém." } }, { "@type": "Question", name: "Posso usar o número em WhatsApp ou num cadastro?", acceptedAnswer: { "@type": "Answer", text: "Não é recomendado. Um exemplo simulado não deve ser usado para contacto, mensagens, validação por SMS, recuperação de conta ou qualquer cadastro externo." } }, { "@type": "Question", name: "Por que escolher o estado e o DDD?", acceptedAnswer: { "@type": "Answer", text: "O DDD dá contexto territorial ao exemplo. A seleção usa a base de DDDs do Meu DDD para apresentar os códigos associados ao estado escolhido." } }, { "@type": "Question", name: "Onde encontro um DDD de verdade?", acceptedAnswer: { "@type": "Answer", text: "Pesquise uma cidade, estado ou código no Meu DDD. As páginas territoriais mostram a cobertura do DDD e ajudam a identificar a área correta." } }] }] };
  }
  ...
  if (guideMatch) {
    ...
    return { title: `${guide.title} | ${site}`, description: guide.description, canonicalPath: path, ogType: "article", ogImage: guide.image, ogImageAlt: guide.imageAlt, jsonLd: [breadcrumbs([{ name: site, item: "/" }, { name: "Guias de telefonia", item: "/guias" }, { name: guide.title, item: path }]), { "@context": "https://schema.org", "@type": "Article", headline: guide.title, description: guide.description, inLanguage: "pt-BR", mainEntityOfPage: path, url: path, image: guide.image, author: { "@type": "Organization", name: site }, publisher: { "@type": "Organization", name: site }, citation: guide.sources.map(sourceId => editorialSources[sourceId].url), about: { "@type": "Thing", name: guide.eyebrow } }, ...(guide.faqs && guide.faqs.length ? [faqPageJsonLd(guide.faqs)] : [])] };
  }
  ...
  if (dddMatch) {
    ...
    const dddFaqs = buildDddFaq({ dddCode: data.code, cityCount: data.cityCount, stateNames: data.states.map(state => state.name) });
    ...
    return { title: dddTitle, description: dddDesc, canonicalPath: path, ogType: "article", jsonLd: [breadcrumbs([{ name: site, item: "/" }, { name: `DDD ${code}`, item: path }]), { "@context": "https://schema.org", "@type": "Article", "@id": `${path}#article`, url: path, headline: `DDD ${code} é de qual estado? ${statePrefix}${data.cityCount} cidades`, description: dddDesc, inLanguage: "pt-BR", mainEntityOfPage: path, datePublished: "2026-08-01", dateModified: "2026-08-01", author: { "@type": "Organization", name: site }, publisher: { "@type": "Organization", name: site }, about: { "@type": "DefinedTermSet", name: "Códigos DDD do Brasil", hasDefinedTerm: { "@type": "DefinedTerm", name: `DDD ${code}`, description: `Código de área com ${data.cityCount} municípios abrangidos.` } } }, { "@context": "https://schema.org", "@type": "CollectionPage", "@id": path, url: path, name: `DDD ${code}: cidades e estados atendidos`, mainEntity: { "@type": "DefinedTerm", name: `DDD ${code}`, description: `Código de área com ${data.cityCount} municípios abrangidos.`, inDefinedTermSet: "Códigos DDD do Brasil" } }, faqPageJsonLd(dddFaqs)] };
  }
  ...
  if (stateMatch) {
    ...
    return { title: `DDD de ${data.state.name}: cidades e códigos | ${site}`, description: `Consulte os DDDs e os ${data.cityCount} municípios de ${data.state.name}, na região ${data.state.region}.`, canonicalPath: path, ogType: "article", jsonLd: [breadcrumbs([{ name: site, item: "/" }, { name: data.state.name, item: path }]), { "@context": "https://schema.org", "@type": "CollectionPage", "@id": path, url: path, name: `DDD de ${data.state.name}`, about: { "@type": "AdministrativeArea", name: data.state.name, identifier: data.state.uf, containedInPlace: { "@type": "Country", name: "Brasil" }, population: data.state.populationEstimated ?? undefined } }, faqPageJsonLd(faqs), { "@context": "https://schema.org", "@type": "Article", "@id": `${path}#article`, url: path, headline: `DDD de ${data.state.name}: cidades e códigos`, description: `Consulte os DDDs e os ${data.cityCount} municípios de ${data.state.name}, na região ${data.state.region}.`, inLanguage: "pt-BR", mainEntityOfPage: path, datePublished: "2026-08-01", dateModified: "2026-08-01", author: { "@type": "Organization", name: site }, publisher: { "@type": "Organization", name: site }, about: { "@type": "AdministrativeArea", name: data.state.name, identifier: data.state.uf, containedInPlace: { "@type": "Country", name: "Brasil" } } }] };
  }
  ...
  if (cityMatch) {
    ...
    const meta = {
      ...
      jsonLd: [
        ...
        faqPageJsonLd(faqs),
        { "@context": "https://schema.org", "@type": "Article", "@id": `${path}#article`, url: path, headline: `DDD de ${data.municipality.name} (${data.state.uf})`, description: `Confira o DDD de ${data.municipality.name}, em ${data.state.name}, e navegue por municípios relacionados.`, inLanguage: "pt-BR", mainEntityOfPage: path, datePublished: "2026-08-01", dateModified: "2026-08-01", author: { "@type": "Organization", name: site }, publisher: { "@type": "Organization", name: site }, about: { "@type": "City", name: data.municipality.name, containedInPlace: { "@type": "AdministrativeArea", name: data.state.name, identifier: data.state.uf } } },
