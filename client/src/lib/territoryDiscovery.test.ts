...
describe("descoberta territorial", () => {
  ...
  it("reduz a precisão da coordenada antes da sugestão quando o modo aproximado é escolhido", () => {
    expect(
      coordinatesForPrecision(
        { latitude: -23.5531, longitude: -46.6321 },
        "approximate"
      )
    ).toEqual({ latitude: -23.5, longitude: -46.75 });
    expect(
      coordinatesForPrecision(
        { latitude: -23.5531, longitude: -46.6321 },
        "exact"
      )
    ).toEqual({ latitude: -23.5531, longitude: -46.6321 });
    expect(precisionDescription("approximate")).toContain(
      "reduzimos a precisão"
    );
  });
});
