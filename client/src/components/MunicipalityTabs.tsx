...
const TABS = [
  ...
  { id: "dining", label: "Bares e restaurantes", icon: Utensils, key: "dining" },
...
export function MunicipalityTabs({ city, stateName, uf, tabs }: Props) {
    ...
    <>
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {tabs.dining.items.map(item => (
                  <a key={item.mapHref} href={item.mapHref} target="_blank" rel="noreferrer"
                    className="group flex gap-3 rounded-2xl border border-[#143d36]/15 bg-[#fffaf1] p-4 transition-colors hover:border-[#f06a4d]/50">
                    <Utensils size={17} className="mt-0.5 shrink-0 text-[#f06a4d]" />
                    <div>
                      <strong className="block font-display text-[#143d36]">{item.name}</strong>
                ...
                {tabs.transport.items.map(item => {
                  const inner = (
                    <div className="group flex gap-3 rounded-2xl border border-[#143d36]/15 bg-[#fffaf1] p-4 transition-colors hover:border-[#f06a4d]/50">
                      <BusFront size={17} className="mt-0.5 shrink-0 text-[#f06a4d]" />
                      <div>
                        <strong className="block font-display text-[#143d36]">{item.name}</strong>
                        <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#f06a4d]">{item.kind}</span>
                        <p className="mt-1 text-sm leading-6 text-[#5d756c]" dangerouslySetInnerHTML={{ __html: renderMarkdownLinks(item.description) }} />
                  ...
                  return item.mapHref ? (
                    <a key={`${item.name}-${item.kind}`} href={item.mapHref} target="_blank" rel="noreferrer">{inner}</a>
                  ) : (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {tabs.climate.details.map(d => (
                  <div key={d.label} className="rounded-2xl bg-[#e9deca] p-4">
                    <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#718378]">{d.label}</span>
                    <p className="mt-1 font-display text-lg text-[#143d36]">{d.value}</p>
                  </div>
                ))}
              </div>
      ...
      <div className="mt-8 rounded-[1.25rem] bg-[#143d36] p-6 text-[#faf3e5] sm:p-8">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#f5c5a1]">Todo o estado</div>
            <p className="mt-2 font-display text-2xl leading-tight">Conheça o estado de {stateName}</p>
            <p className="mt-2 max-w-xl text-sm leading-6 text-[#b8cec4]">
              {stateName} reúne todos os municípios e códigos DDD cobertos nesta página. Explore o território completo na página do estado.
            </p>
            <Link href={`/estado/${uf.toLowerCase()}`} className="mt-4 inline-flex items-center gap-1 font-bold text-[#f5c5a1] hover:text-[#faf3e5]">
  ...
  return (
    <section className="container py-10 lg:py-14" aria-label={`Guia local de ${city}`}>
      <div className="mx-auto max-w-4xl">
        <div className="mb-8">
          <div className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#f06a4d]">Guia local · Revisado em {tabs.reviewedOn}</div>
          <h2 className="mt-3 font-display text-4xl tracking-[-0.03em] text-[#143d36] sm:text-5xl">
            {city} em detalhe
          </h2>
        </div>
        <div role="tablist" aria-label="Secções do guia local" className="mb-6 flex flex-wrap gap-2">
          {TABS.map(({ id, label, icon: Icon }) => (
            <button key={id} role="tab" id={`tab-${id}`} aria-selected={active === id} aria-controls={`tabpanel-${id}`}
        ...
        <p className="mt-6 text-xs leading-5 text-[#718378]">
          Conteúdo editorial baseado nos dados territoriais oficiais (IBGE e ANATEL) e em fontes locais públicas. Os links abrem pesquisas no Google Maps para confirmação da localização.
        </p>
      </div>
    </section>
  );
