...
export default function SuggestionModerationPage() {
  ...
  const suggestions = trpc.insights.localitySuggestions.useQuery(queryInput, {
    enabled: isAdmin,
  });
        ...
        <header className="mb-7 flex flex-col gap-3 border-b border-[#143d36]/10 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 font-serif text-4xl tracking-tight">
              Moderar sugestões locais
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#143d36]/70">
              Reveja contribuições sobre mobilidade e telefones úteis antes de
              qualquer atualização editorial. Os envios não incluem dados de
              contacto.
            </p>
          </div>
        ...
        {!loading && !isAdmin ? (
          <Card className="border-[#e8533a]/20 bg-[#fffaf0] shadow-none">
            <CardHeader>
              ...
              <CardDescription>
                A moderação de contribuições locais está disponível apenas para
                a equipa administradora.
              </CardDescription>
            </CardHeader>
          </Card>
        ) : null}
          ...
          <div className="grid gap-4">
              ...
              <CardContent className="grid gap-4 p-5 sm:grid-cols-3">
                ...
                <label className="grid gap-2 text-sm font-semibold">
                  ...
                  <select
                    value={topicFilter}
                    onChange={event =>
                      setTopicFilter(event.target.value as typeof topicFilter)
                    }
                    className="h-10 rounded-lg border border-[#143d36]/15 bg-white px-3 text-sm font-normal"
                  >
            ...
            {suggestions.error ? (
              <div className="rounded-2xl border border-[#e8533a]/20 bg-[#fffaf0] p-6 text-sm text-[#9c2b1a]">
                Não foi possível carregar as sugestões. Verifique a sessão
                administrativa.
              </div>
            ) : null}
            {suggestions.data?.length === 0 ? (
              <Card className="border-[#143d36]/10 shadow-none">
                <CardContent className="flex gap-3 p-6 text-sm text-[#143d36]/70">
                  <MessageSquareText className="h-5 w-5 shrink-0 text-[#e8533a]" />
                  Não existem sugestões neste estado de moderação.
                </CardContent>
              </Card>
            ) : null}
            {suggestions.data?.map(item => (
              <Card key={item.id} className="border-[#143d36]/10 shadow-none">
                <CardContent className="grid gap-4 p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold">
                        {item.municipalityName ??
                          `Município ${item.municipalityIbgeCode}`}
                        {item.uf ? ` · ${item.uf}` : ""}
                      </p>
                    ...
                    <div className="flex gap-2">
                      <Badge
                        variant="outline"
                        className="border-[#e8533a]/30 text-[#b63e29]"
                      >
                        {labels[item.topic]}
                      </Badge>
                      <Badge
                        variant="outline"
                        className="border-[#143d36]/20 text-[#143d36]"
                      >
                        {statuses[item.status]}
                      </Badge>
                    </div>
                  </div>
                  <p className="rounded-xl bg-[#faf3e5] p-4 text-sm leading-6">
                    {item.note}
                  </p>
                  {item.status !== "approved" && item.status !== "dismissed" ? (
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() =>
                          review.mutate({ id: item.id, status: "reviewed" })
                        }
                      >
                      ...
                      <Button
                        size="sm"
                        className="bg-[#143d36] text-[#faf3e5] hover:bg-[#0d2d27]"
                        disabled={review.isPending}
                        onClick={() =>
                      ...
                      <Button
                        ...
                        disabled={review.isPending}
                        onClick={() =>
                          review.mutate({ id: item.id, status: "dismissed" })
                        }
                      >
