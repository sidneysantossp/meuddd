...
export default function DddDetail() {
  ...
  const copyPageLink = () => copyText(`${window.location.origin}/ddd/${code}`, "Link do DDD copiado");
  const faqs = [
  ...
  return <main className="page-shell min-h-screen bg-[#faf3e5] text-[#143d36]">
    <header className="border-b border-[#d9d1bf] bg-[#faf3e5]/90 backdrop-blur"><div className="container flex min-h-[78px] items-center justify-between gap-4"><Link href="/#buscar" className="inline-flex items-center gap-2 text-sm font-bold text-[#143d36]"><ArrowLeft size={17} /> DDD Brasil</Link><button onClick={copyPageLink} className="pressable inline-flex items-center gap-2 rounded-full border border-[#cfc3b0] px-4 py-2 text-xs font-bold"><Share2 size={14} /> Copiar link</button></div></header>
