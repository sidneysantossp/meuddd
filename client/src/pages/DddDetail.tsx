...
export default function DddDetail() {
  ...
  return <main className="page-shell min-h-screen bg-[#faf3e5] text-[#143d36]">
    ...
    <section className="container grid gap-12 py-14 lg:grid-cols-[0.72fr_1.28fr] lg:py-20">
      <div><div className="mb-5 text-[10px] font-bold uppercase tracking-[0.24em] text-[#f06a4d]">Atlas de conexão</div><div className="font-display text-[clamp(6rem,18vw,13rem)] leading-[0.72] tracking-[-0.1em] text-[#f06a4d]">{data.code}</div><div className="mt-7 flex items-center gap-2 text-sm font-bold uppercase tracking-[0.16em] text-[#668077]"><Phone size={15} className="text-[#f06a4d]" /> Código de área</div><h1 className="mt-5 font-display text-4xl leading-[0.94] tracking-[-0.06em] sm:text-5xl">DDD {data.code}: {states[0] ? `${states[0].name} — ` : ""}{data.cityCount} cidades atendidas</h1><p className="mt-5 max-w-md text-sm leading-6 text-[#6b8177]">Este é o link direto e partilhável do DDD {data.code}. Envie-o para consultar as localidades cobertas pelo código{states.length ? states.map((state, index) => <span key={state.uf}>{index === 0 ? " nos estados de " : index === states.length - 1 ? " e " : ", "}<Link href={`/estado/${state.uf.toLowerCase()}`} className="font-bold text-[#143d36] underline decoration-[#f06a4d]/40 underline-offset-2">{state.name}</Link></span>) : ""} e navegue ao pilar estadual.</p><div className="mt-8 flex flex-wrap gap-2">{states.map(state => <Link key={state.uf} href={`/estado/${state.uf.toLowerCase()}`} className="rounded-full bg-[#e9deca] px-3 py-2 text-xs font-bold text-[#143d36]">{state.name} · {state.uf}</Link>)}</div></div>
      <div className="rounded-[1.75rem] border border-[#ded4c3] bg-[#fffaf1] p-5 shadow-[0_20px_50px_rgba(20,61,54,0.08)] sm:p-7"><div className="mb-6 flex items-end justify-between border-b border-[#e6ddce] pb-5"><div><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#7b9085]">Municípios abrangidos</div><h2 className="mt-2 font-display text-3xl tracking-[-0.04em]">Lista completa</h2></div><span className="grid size-10 place-items-center rounded-full bg-[#f5c5a1]" aria-label="DDD"><Copy size={16} /></span></div><div className="ddd-mun-grid">{municipalities.map(municipality => <Link key={municipality.ibgeCode} href={`/cidade/${municipality.uf.toLowerCase()}/${municipality.slug}`} className="ddd-mun-item" data-uf={municipality.uf} data-capital={municipality.capital ? "1" : undefined}>{municipality.name} <small><span aria-hidden="true">&#9679;</span> {municipality.uf} {municipality.capital ? "· capital" : ""}</small><span aria-hidden="true">&#8599;</span></Link>)}</div></div>
    </section>
