...
export default function StatePage() {
  ...
  return <main className="page-shell min-h-screen bg-[#faf3e5] text-[#143d36]">
    <PublicNavbar />
    <section className="container py-14 lg:py-20"><div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr]"><div><div className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#f06a4d]">Estado / {state.region}</div><h1 className="mt-4 font-display text-6xl leading-[0.9] tracking-[-0.07em] sm:text-7xl">DDD de {state.name}</h1><p className="mt-6 max-w-xl text-base leading-7 text-[#5d756c]">Consulte todos os códigos de área e municípios de {state.name}. Esta página reúne a navegação territorial e os dados de referência para a busca telefónica no estado. A numeração do território brasileiro é regulada pela <a href={OFFICIAL_URLS.anatelNumeracao} target="_blank" rel="noreferrer" className="underline decoration-[#f06a4d]/60 underline-offset-4 font-semibold text-[#143d36] hover:text-[#f06a4d]">ANATEL</a>, no âmbito do Plano de Numeração Brasileiro.</p><div className="mt-8 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-[#e9deca] p-4"><Users size={17} className="text-[#f06a4d]" /><strong className="mt-4 block font-display text-3xl">{formatPopulation(state.populationEstimated)}</strong><a href={`https://www.ibge.gov.br/cidades-e-estados/${state.uf.toLowerCase()}.html`} target="_blank" rel="noreferrer" className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#718378] hover:text-[#f06a4d]">habitantes · IBGE {state.populationReferenceYear}</a></div><div className="rounded-2xl bg-[#143d36] p-4 text-[#faf3e5]"><Phone size={17} className="text-[#f5c5a1]" /><strong className="mt-4 block font-display text-3xl">{ddds.length}</strong><span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#b8cec4]">DDDs no estado</span></div></div></div><div><BrazilStateMap states={states.data ?? []} selectedUf={state.uf} onStateSelect={nextUf => setLocation(`/estado/${nextUf.toLowerCase()}`)} /></div></div></section>
    ...
    <section className="container py-12 lg:py-16" aria-labelledby={`ddd-de-${state.uf.toLowerCase()}-h2`}>
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#f06a4d]">Resposta direta</div>
          <h2 id={`ddd-de-${state.uf.toLowerCase()}-h2`} className="mt-3 font-display text-4xl tracking-[-0.05em] sm:text-5xl">Qual é o DDD de {state.name}?</h2>
          <p className="mt-5 max-w-2xl text-base leading-7 text-[#5d756c]">{state.name} é atendido pelos DDDs {ddds.map(ddd => ddd.code).join(", ")}. Cada código cobre uma região distinta do estado: os maiores volumes de municípios concentram-se {ddds.length > 0 ? `no DDD ${ddds[0].code}` : "nas capitais e nos principais centros urbanos"}. Para descobrir o código de uma cidade específica, use a lista de municípios desta página ou a busca do topo — o DDD será exibido junto ao resultado.</p>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-[#6b8177]">Ao ligar para outro número de {state.name} fora da mesma área de numeração, é preciso discar 0 + código da prestadora + DDD + número. Para ligações de longa distância, verifique o plano da sua operadora para conhecer as tarifas aplicáveis.</p>
        </div>
        <div className="flex flex-wrap gap-2 lg:justify-end">
          {ddds.map(ddd => <Link key={ddd.code} href={`/ddd/${ddd.code}`} className="pressable rounded-full bg-[#143d36] px-4 py-2.5 text-sm font-bold text-[#faf3e5]">DDD {ddd.code} · {ddd.cityCount} {ddd.cityCount === 1 ? "cidade" : "cidades"}</Link>)}
        </div>
      </div>
    </section>
    ...
    <section className="container py-16"><div className="mb-8"><div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#f06a4d]">Municípios / {cityCount}</div><h2 className="mt-3 font-display text-4xl tracking-[-0.05em]">Todas as cidades de {state.name}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#6b8177]">Navegue diretamente para a página da sua cidade, com DDD, população estimada, coordenadas e links territoriais relacionados.</p></div><div className="mun-grid">{municipalities.map(city => <Link key={city.ibgeCode} href={`/cidade/${city.uf.toLowerCase()}/${city.slug}`} className="mun-item" data-ddd={city.ddd}>{city.name} <small>{city.ddd}</small><span aria-hidden="true">&#8599;</span></Link>)}</div></section>
    <FaqSection id={`faq-estado-${state.uf.toLowerCase()}`} heading="Perguntas frequentes" subheading={`DDD e telefonia em ${state.name}`} faqs={faqs} theme="dark" />
