...
export default function MunicipalityPage() {
  ...
  return <main className="page-shell min-h-screen bg-[#faf3e5] text-[#143d36]">
    ...
    <section className="border-y border-[#ded4c3] bg-[#fffaf1]"><div className="container py-14"><div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]"><div><div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#f06a4d]">Território ligado</div><h2 className="mt-3 font-display text-4xl tracking-[-0.05em]">Outras cidades com DDD {municipality.ddd}</h2><p className="mt-4 text-sm leading-6 text-[#6b8177]">Explore localidades do mesmo código de área e volte ao pilar de {state.name} para ver todos os DDDs do estado.</p><button type="button" onClick={() => setLocation(`/estado/${state.uf.toLowerCase()}`)} className="mt-5 text-sm font-bold text-[#f06a4d]">Ver todas as cidades de {state.uf} →</button></div><div className="grid gap-x-6 border-t border-[#e1d6c4] sm:grid-cols-2">{relatedMunicipalities.map(city => <Link key={city.ibgeCode} href={`/cidade/${city.uf.toLowerCase()}/${city.slug}`} className="group flex items-center justify-between gap-3 border-b border-[#e1d6c4] py-3 text-sm font-semibold hover:text-[#f06a4d]"><span>{city.name}</span><ArrowUpRight size={14} className="shrink-0" /></Link>)}</div></div></div></section>
