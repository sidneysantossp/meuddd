...
vi.mock("@/components/BrazilStateMap", () => ({
  BrazilStateMap: () => <div data-testid="state-map" />,
}));
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ddd: {
      ...
      resolveNearbyTerritory: {
        useMutation: () => ({
          mutate: (
            _input: unknown,
            options: { onSuccess: (territory: Territory) => void }
          ) => {
            mocks.onNearbySuccess = options.onSuccess;
          },
        }),
      },
    },
...
describe("feedback de localização na página inicial", () => {
  ...
  it("exibe o progresso enquanto resolve o território e permite limpar a sugestão", async () => {
    ...
    const locateButton = Array.from(container.querySelectorAll("button")).find(
      button => button.textContent?.includes("Usar localização aproximada")
    );
    expect(locateButton).toBeTruthy();
    expect(
      Array.from(container.querySelectorAll("button"))
        .find(button =>
          button.textContent?.includes("Aproximada (recomendada)")
        )
    ...
    expect(container.textContent).toContain(
      "Localização aproximada: São Paulo · SP. Sugerimos o DDD 11."
    );
    const clearButton = Array.from(container.querySelectorAll("button")).find(
      button => button.textContent?.includes("Limpar sugestão")
    );
