...
describe("submissão controlada IndexNow", () => {
  ...
  it("rejeita URLs externas para evitar notificações indevidas", () => {
    expect(() => createIndexNowPayload(["https://example.org/pagina"])).toThrow(
      "host canónico"
    );
  });
  ...
  it("envia um único POST JSON ao endpoint global", async () => {
    ...
    await expect(
      submitIndexNowUrls(["https://www.meuddd.com.br/"], request)
    ).resolves.toEqual({ accepted: true, status: 202, submitted: 1 });
