...
describe("configuração de publicação Vercel", () => {
  it("mantém uma aplicação Express e inclui os artefactos necessários para SSR", () => {
    ...
    const coreEntry = fs.readFileSync(
      path.join(projectRoot, "server/_core/index.ts"),
      "utf8"
    );
    const ssrStatic = fs.readFileSync(
      path.join(projectRoot, "server/_core/ssrStatic.ts"),
      "utf8"
    );
    const runtimeVerification = fs.readFileSync(
      path.join(projectRoot, "scripts/verify-vercel-runtime.mjs"),
      "utf8"
    );
