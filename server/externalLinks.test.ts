...
describe("política de links externos", () => {
  it("permite domínios governamentais, Google Maps e climate-data.org", () => {
    ...
    expect(isAllowedExternal("https://osasco.sp.gov.br/feiras-livres/")).toBe(
      true
    );
    expect(isAllowedExternal("https://penedo.al.gov.br/x")).toBe(true);
    expect(
      isAllowedExternal("https://www.ibge.gov.br/cidades-e-estados/sp.html")
    ).toBe(true);
    expect(
      isAllowedExternal(
        "https://cidades.ibge.gov.br/v4/brasil/sp/osasco/panorama"
      )
    ).toBe(true);
    expect(
      isAllowedExternal("https://www.google.com/maps/search/?api=1&query=x")
    ).toBe(true);
    expect(
      isAllowedExternal(
        "https://pt.climate-data.org/america-do-sul/brasil/sp/osasco/"
      )
    ).toBe(true);
    expect(
      isAllowedExternal(
        "https://en.climate-data.org/south-america/brazil/sp/osasco-1/"
      )
    ).toBe(true);
  });
  ...
  it("rejeita portais comerciais e de terceiros", () => {
    ...
    expect(isAllowedExternal("https://www.metrocptm.com.br/linha-9/")).toBe(
      false
    );
    ...
    const input = {
      tourism: {
        items: [
          {
            name: "Praça",
            description: "[site oficial](https://osasco.sp.gov.br/x)",
          },
        ],
      },
      transport: {
        items: [
          {
            name: "Aeroporto",
            description: "perto",
            mapHref: "https://www.google.com/maps/search/?api=1&query=x",
          },
        ],
      },
      climate: {
        body: "[clima](https://pt.climate-data.org/america-do-sul/brasil/sp/osasco/)",
        source: { label: "Climatempo", href: "https://www.climatempo.com.br/" },
      },
    };
    const out = sanitizeExternalLinks(input) as any;
    expect(out.tourism.items[0].description).toContain("site oficial");
    expect(out.transport.items[0].mapHref).toBe(
  ...
  it("OFFICIAL_URLS gera URLs válidas por UF e cidade", () => {
    expect(OFFICIAL_URLS.ibgeCity("SP", "osasco")).toBe(
      "https://cidades.ibge.gov.br/v4/brasil/sp/osasco/panorama"
    );
    expect(OFFICIAL_URLS.climateData("SP", "osasco")).toContain(
      "pt.climate-data.org/america-do-sul/brasil/sp/osasco"
    );
